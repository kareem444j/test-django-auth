...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'username', 'password', 'confirm_password', 'email'] # include confirm password field
        extra_kwargs = {
            "password": {
                "write_only": True,
...
class PasswordChangeSerializer(serializers.Serializer):
    ...
    def validate_new_password(self, value):
        if not value or not value.strip():
            raise serializers.ValidationError("New password field cannot be empty or whitespace.")
        if len(value) < 8:
